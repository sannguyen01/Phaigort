  ...
  return (
    <div className="pt-16 md:pt-20">
      <Container className="max-w-3xl">
        ...
        <Body className="mt-8 text-silver">The profound understanding that exceptional materials spanning geological time, cultural heritage, and human innovation deserve equal reverence. Not a marketing concept but an empirical worldview connecting visible beauty to invisible forces.</Body>
      </Container>
      <Divider />
      <Container className="max-w-3xl">
        <section className="space-y-16">
          <div>
            ...
            <Body className="mt-6 text-silver">Phaigort fuses Greek phainomenon — that which reveals itself through observation — and Portuguese fazer — to make, to create. Honoring Garcia de Orta, the Portuguese physician who wrote the first scientific gemstone book in everyday language &ldquo;so that traders and other locals could make use of it&rdquo; instead of keeping knowledge exclusive.</Body>
            <Body className="mt-4 text-silver">Our name captures our mission: reveal what Earth creates, celebrate what humans make, and share the knowledge generously.</Body>
          </div>
          <Divider className="my-0" />
          <div>
            <Caption className="text-coral">Heritage</Caption>
            <H2 className="mt-6">The Iberian Voyage</H2>
            <Body className="mt-6 text-silver">Drawing DNA from 16th-century Portuguese and Spanish trading vessels where materials from four continents converged in markets valuing knowledge over pedigree. These nau ships were not instruments of conquest but vehicles of discovery — carrying materials, knowledge, and people across uncharted waters.</Body>
            <Body className="mt-4 text-silver">We inherit this spirit: the intellectual rigor and geometric clarity, the voyage romance and artisan reverence. Where great luxury houses built fortresses, we build harbors. Where they gatekeep, we welcome. Where they whisper &ldquo;for the chosen few,&rdquo; we declare &ldquo;for the genuinely curious.&rdquo;</Body>
          </div>
          <Divider className="my-0" />
          <div>
            ...
            <div className="mt-8 space-y-8">
              {[
                ...
                <div key={item.label} className="pl-6 border-l border-coral/30">
                  <h3 className="font-heading text-xl text-platinum">{item.label}</h3>
                  <Body className="mt-2 text-silver">{item.text}</Body>
                </div>
              ))}
            </div>
          </div>
