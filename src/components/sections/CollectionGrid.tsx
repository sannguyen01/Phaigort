...
export function CollectionGrid() {
  ...
  return (
    <section ref={ref} className="py-32 md:py-48 bg-navy">
      <Container>
        <motion.div initial={{ opacity: 0, y: 32 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 1, ease: [0.22, 1, 0.36, 1] }} className="text-center mb-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {TREASURE_DOMAINS.map((domain, i) => (
            <motion.div key={domain.title} initial={{ opacity: 0, y: 24 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.1 + i * 0.12, duration: 0.8, ease: [0.22, 1, 0.36, 1] }}>
              <Link href={domain.href} className="group block p-8 md:p-12 bg-deep-navy border border-silver/10 hover:border-coral/30 transition-all duration-500">
                <span className="font-body text-xs uppercase tracking-widest text-coral/60 group-hover:text-coral transition-colors duration-300">{String(i + 1).padStart(2, "0")}</span>
                ...
                <span className="mt-8 inline-flex items-center gap-2 font-body text-xs uppercase tracking-widest text-coral/50 group-hover:text-coral transition-colors duration-300">
